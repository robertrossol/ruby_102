...
module DoesItRock
  ...
  class Score
    ...
    #only rescue ZeroDivisionError
    rescue ZeroDivisionError
      #return an instance of NoScore, instead of just the class name, because...(see next note)
      NoScore.new
    end
  end
  ...
  def self.find_out(term)
    ...
    case score
    #in case statements, when compares via === and with class name (NoScore) it essentially checks if score is an instance of NoScore
    when NoScore
      "No idea about #{term}..."
    when 0...0.5
      "#{term} is not fun."
